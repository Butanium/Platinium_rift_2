...
let score z_start z_end =
    ...
    float_of_int(z_end.ker_dist) /. !maxDist *. 10. +. (if z_end.owner = 1-myid then 10. else (if z_end.owner = -1
        ...
        float_of_int(z_end.pods.(myid)) +. z_end.bonus *. 10. +. (if z_end.ker_dist > 0 then 0. else 0. )
...
while true do
    ...
    let rec crossPods l = match l with
        ...
        for _ = 1 to x.pods.(myid) do
            ...
            crossZ ys;
            if !enemies>0 && (score x x) +. float_of_int(!enemies*2) +. 10.  > !bestScore then (
                x.incomingPods <- x.incomingPods + 1;
                prerr_endline (Printf.sprintf "%f < %f" !bestScore ((score x x) +. float_of_int(!enemies*2) +. 10.))
            )
            else (
                if x.ker_dist = 0 then (
                    prerr_endline (Printf.sprintf "%f > %f" !bestScore(score x y));
                    prerr_endline (Printf.sprintf "%d len" (y.ker_dist))
                );
                x.incomingPods <- x.incomingPods - 1;
