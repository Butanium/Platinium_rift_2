...
let playercount, myid, zonecount, linkcount =
  Scanf.sscanf (input_line stdin) " %d  %d  %d  %d"
    (fun playercount myid zonecount linkcount ->
      (playercount, myid, zonecount, linkcount))
...
let update_zone zone owner pod0 pod1 visible =
  ...
  if owner = 1 - myid then (
    if zone.malus then
      prerr_endline (Printf.sprintf "/!!!!!! %d !!!!!!" zone.id);
    zone.malus <- false)
...
let rec add_bonus zone bonus =
  if bonus *. 2. > zone.bonus && bonus > minB then (
    ...
    let rec cross l =
      ...
      | [] -> ()
      | x :: xs ->
          add_bonus x (bonus /. ratio);
          cross xs
    in
    cross !zoneLinkR.(zone.id))
...
let score z_start z_end podsCount =
  ...
  if z_end.pods.(1 - myid) > 0 then (
    add_bonus z_end 1.;
    z_start.malus <- false;
    z_end.malus <- false);
  if not z_end.malus then
    (float_of_int z_end.ker_dist /. !maxDist *. 10.)
    +. (if z_end.owner = 1 - myid then 10.
       else if z_end.owner = -1 then
         if z_end.incomingPods = 0 then 100. else 10.
       else 0.)
    -. float_of_int (z_end.incomingPods + z_end.pods.(myid))
       /. float_of_int (podsCount + 1)
       /. !maxDist *. 10.
    +. (z_end.bonus *. 10.)
    +. if z_end.ker_dist > 0 then 0. else 0.
...
for i = 0 to zonecount - 1 do
  ...
  let zoneid, platinumsource =
    Scanf.sscanf (input_line stdin) " %d  %d" (fun zoneid platinumsource ->
        (zoneid, platinumsource))
  in
  ()
done;
...
for i = 0 to linkcount - 1 do
  let zone1, zone2 =
    Scanf.sscanf (input_line stdin) " %d  %d" (fun zone1 zone2 ->
        (zone1, zone2))
  in
...
while true do
  ...
  let myplatinum = int_of_string (input_line stdin) in
  (* your available Platinum *)
  for i = 0 to zonecount - 1 do
    (* zid: this zone's ID *)
    ...
    let zid, ownerid, podsp0, podsp1, visible, platinum =
      Scanf.sscanf (input_line stdin) " %d  %d  %d  %d  %d  %d"
        (fun zid ownerid podsp0 podsp1 visible platinum ->
          (zid, ownerid, podsp0, podsp1, visible, platinum))
    in
    if !first then (
  ...
  if !first then
    zoneLinkR :=
      Array.map (fun l -> List.map (fun x -> zones.(x)) l) tempZoneLink;
  let rec crossPods l =
    match l with
    | [] -> ()
    | x :: xs ->
        ...
        for i = 1 to x.pods.(myid) do
          ...
          List.iter
            (fun x -> podCount += (x.pods.(myid) + abs x.incomingPods))
            !zoneLinkR.(x.id);
          let (y :: ys) = !zoneLinkR.(x.id) in
          let enemies = ref y.pods.(1 - myid)
          and new_id = ref y.id
          and bestScore = ref (score x y !podCount) in
          if !isIsolated && y.ker_dist > x.ker_dist && not y.malus then
            isIsolated := false;
          let rec crossZ t =
            match t with
            | [] -> ()
            | z :: zs ->
                let sc = score x z !podCount in
                if sc > !bestScore then (
                  bestScore := sc;
                  new_id := z.id);
                enemies += z.pods.(1 - myid);
                if !isIsolated && z.ker_dist > x.ker_dist && not z.malus then
                  isIsolated := false;
                crossZ zs
          in
          crossZ ys;
          ...
          let idleScore =
            score x x !podCount
            +. float_of_int (!enemies - (x.incomingPods + x.pods.(myid)))
            +. 10.
            +. (10. *. zones.(!new_id).bonus)
          in
          if
            not
              ((2 * !enemies) - (x.incomingPods + x.pods.(myid)) >= 0
              && idleScore > !bestScore)
          then (
